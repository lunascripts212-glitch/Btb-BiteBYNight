...
local function axeStartLock()
    if axeLockActive then return end
    ...
    -- Use Heartbeat + CFrame.lookAt (same as HDT legit mode) — 
    -- this survives physics corrections that RenderStepped CFrame writes don't
    axeLockConn = RunService.Heartbeat:Connect(function()
        local elapsed = tick() - startTime
        if elapsed >= AXE_DURATION or not axeLockEnabled or not axeLockActive
