...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabSurSen = win:Tab({ Title = "Survivors/Sentinels", Icon = "users" })
...
-- Guest1337 (loads external as before — no script provided)
secSentinels:Button({ Title="Guest1337", Callback=function()
    loadstring(game:HttpGet("https://pastebin.com/raw/Kx5U4bLL"))()
end })
...
-- ──────────────────────────────────────────────
-- JANE DOE / V1PERTHROW (inline)
-- ──────────────────────────────────────────────
do
    local janeSection = tabSurSen:Section({ Title = "Jane Doe (V1perThrow)", Opened = false })

    local vtPlayers           = game:GetService("Players")
    local vtRunService        = game:GetService("RunService")
    local vtReplicatedStorage = game:GetService("ReplicatedStorage")
    local vtUserInputService  = game:GetService("UserInputService")
    local vtLocalPlayer       = vtPlayers.LocalPlayer
    local vtCamera            = workspace.CurrentCamera

    local vtActors  = nil; pcall(function() vtActors  = require(vtReplicatedStorage.Modules.Actors)  end)
    local vtNetwork = nil; pcall(function() vtNetwork = require(vtReplicatedStorage.Modules.Network) end)
    local vtUtil    = nil; pcall(function() vtUtil    = require(vtReplicatedStorage.Modules.Util)    end)

    local vtIsMobile = vtUserInputService.TouchEnabled and not vtUserInputService.KeyboardEnabled
    local vtEnabled  = false
    local vtAimbotOn = false
    local vtPatched  = false
    local vtCrystalCB= nil
    local vtUnloaded = false

    local vtAimOffset   = -0.3
    local vtPrediction  = 0.6
    local vtHoldDur     = 0.9

    local vtKillerMotion = {}
    local function vtGetKillerVel(hrp)
        local now=tick(); local pos=hrp.Position; local data=vtKillerMotion[hrp]
        if not data then vtKillerMotion[hrp]={lastPos=pos,lastTime=now,velocity=Vector3.zero}; return Vector3.zero end
    ...
    local function vtGetNearestKiller(fromPos)
        local folder=workspace:FindFirstChild("Players"); folder=folder and folder:FindFirstChild("Killers"); if not folder then return nil end
    ...
    local function vtSetShiftLock(on)
        if vtIsMobile then return end
        vtUserInputService.MouseBehavior = on and Enum.MouseBehavior.LockCenter or Enum.MouseBehavior.Default
    end
    local function vtSetMouseBehavior(b)
        if vtIsMobile then return end
        vtUserInputService.MouseBehavior = b
    end

    local function vtAimFirePC(myHRP, killerHRP, fireCB, holdDur, projLifetime)
        local origCT=vtCamera.CameraType
        local vp=vtCamera.ViewportSize; pcall(mousemoveabs, vp.X/2, vp.Y/2)
        vtSetMouseBehavior(Enum.MouseBehavior.LockCurrentPosition)
        vtCamera.CameraType=Enum.CameraType.Scriptable
        local vel=vtGetKillerVel(killerHRP)
        local ap=killerHRP.Position+vel*vtPrediction
        ap=Vector3.new(ap.X,vtCamera.CFrame.Position.Y+vtAimOffset,ap.Z)
        vtCamera.CFrame=CFrame.lookAt(vtCamera.CFrame.Position,ap)
        vtRunService.RenderStepped:Wait(); vtRunService.RenderStepped:Wait()
        vtSetShiftLock(true); fireCB()
        local st=tick(); local slDis=false; local smoothVel=vtGetKillerVel(killerHRP)
        local conn; conn=vtRunService.RenderStepped:Connect(function(dt)
            if not vtAimbotOn or not myHRP or not myHRP.Parent then
                conn:Disconnect(); vtCamera.CameraType=origCT; vtSetMouseBehavior(Enum.MouseBehavior.Default)
                if not slDis then vtSetShiftLock(false); slDis=true end; return
            end
            local el=tick()-st; local rv=vtGetKillerVel(killerHRP); smoothVel=smoothVel:Lerp(rv,math.clamp(dt*8,0,1))
            local a=killerHRP.Position+smoothVel*vtPrediction
            a=Vector3.new(a.X,vtCamera.CFrame.Position.Y+vtAimOffset,a.Z)
            vtCamera.CFrame=CFrame.lookAt(vtCamera.CFrame.Position,a)
            if el>=holdDur then
                conn:Disconnect(); vtCamera.CameraType=origCT; vtSetMouseBehavior(Enum.MouseBehavior.Default)
                if not slDis then
                    local rem=projLifetime-el
                    if rem>0 then task.delay(rem,function() if not slDis then vtSetShiftLock(false); slDis=true end end)
                    else vtSetShiftLock(false); slDis=true end
                end
            end
        end)
    end

    local function vtAimFireMobile(myHRP, killerHRP, fireCB, holdDur, projLifetime, abCfg)
        local origCT=vtCamera.CameraType; vtCamera.CameraType=Enum.CameraType.Scriptable
        local v0=abCfg.MaxSpeed; local g=abCfg.ProjectileArc
        ...
        local smoothVel=vtGetKillerVel(killerHRP)
        local function buildCF(vel)
            local sp=myHRP.CFrame.Position+Vector3.new(0,v238,0)
            local pred=killerHRP.Position+vel*vtPrediction+Vector3.new(0,vtAimOffset,0)
            local delta=pred-sp; local flatV=Vector3.new(delta.X,0,delta.Z)
            local dx=flatV.Magnitude; local dy=delta.Y
            if dx<0.01 then
                local s=dy>=0 and Vector3.new(0,1,0) or Vector3.new(0,-1,0)
                return CFrame.new(vtCamera.CFrame.Position,vtCamera.CFrame.Position+s)
            end
            local flatDir=flatV.Unit
            local v2=v0*v0; local v4=v2*v2; local disc=v4-g*(g*dx*dx+2*dy*v2)
            local theta
            if disc<0 then theta=math.atan2(dy,dx)
            else theta=math.atan2(v2-math.sqrt(disc),g*dx) end
            local T=math.tan(theta); local denom=3+T; local alpha
            if math.abs(denom)<0.0001 then alpha=-math.pi/2
            else alpha=math.atan2(3*T-1,denom) end
            local yawCF=CFrame.new(vtCamera.CFrame.Position,vtCamera.CFrame.Position+flatDir)
            return yawCF*CFrame.Angles(alpha,0,0)
        end
        vtCamera.CFrame=buildCF(smoothVel)
        vtRunService.RenderStepped:Wait(); vtRunService.RenderStepped:Wait()
        fireCB()
        local st=tick(); local conn
        conn=vtRunService.RenderStepped:Connect(function(dt)
            if not vtAimbotOn or not myHRP or not myHRP.Parent then
                conn:Disconnect(); vtCamera.CameraType=origCT; return
            end
            local rv=vtGetKillerVel(killerHRP); smoothVel=smoothVel:Lerp(rv,math.clamp(dt*8,0,1))
            vtCamera.CFrame=buildCF(smoothVel)
            if tick()-st>=holdDur then conn:Disconnect(); vtCamera.CameraType=origCT end
        end)
    end

    local function vtAimAndFire(myHRP, fireCB, holdDur, projLifetime, abCfg)
        local killer=vtGetNearestKiller(myHRP.Position); if not killer then fireCB(); return end
        local killerHRP=killer:FindFirstChild("HumanoidRootPart"); if not killerHRP then fireCB(); return end
        if vtIsMobile then vtAimFireMobile(myHRP,killerHRP,fireCB,holdDur,projLifetime,abCfg)
        else vtAimFirePC(myHRP,killerHRP,fireCB,holdDur,projLifetime) end
    end

    local function vtGetLocalActor()
        if not vtActors then return nil end
        for _,actor in vtActors.CurrentActors do
            if actor.Player==vtLocalPlayer then return actor end
        end
        return nil
    end

    local function vtApplyPatch(actor)
        if vtPatched or not actor or not actor.Behavior then return end
        if not actor.Behavior.Abilities or not actor.Behavior.Abilities.Crystal then return end
        vtCrystalCB=actor.Behavior.Abilities.Crystal.Callback
        actor.Behavior.Abilities.Crystal.Callback=function(self,p290)
            if vtRunService:IsServer() then return vtCrystalCB(self,p290) end
            if not vtEnabled then return vtCrystalCB(self,p290) end
            if p290=="Cancelled" or not p290 then return vtCrystalCB(self,p290) end
            self.State.IsCrystalEquipped=true
            if vtUtil then pcall(function() vtUtil:ToggleLockForAbilityIcon("Axe","JaneDoeCrystalEquipped",true) end) end
            task.spawn(function()
                local abCfg=self.Config.Crystal
                task.wait(abCfg.ChannelTime+0.05)
                if not (self.State.IsCrystalEquipped and vtEnabled) then return end
                local function doFire()
                    if vtNetwork then
                        vtNetwork:FireServerConnection(("%sCrystalInput"):format(self.Player.Name),"REMOTE_EVENT",1)
                    end
                end
                if vtAimbotOn then
                    local myHRP=self.Rig and self.Rig:FindFirstChild("HumanoidRootPart")
                    if myHRP then vtAimAndFire(myHRP,doFire,vtHoldDur,abCfg.ProjectileLifetime,abCfg)
                    else doFire() end
                else doFire() end
            end)
        end
    ...
    local function vtRemovePatch(actor)
        if not vtPatched or not actor then return end
        if not actor.Behavior or not actor.Behavior.Abilities or not actor.Behavior.Abilities.Crystal then return end
        if vtCrystalCB then actor.Behavior.Abilities.Crystal.Callback=vtCrystalCB; vtCrystalCB=nil end
        vtPatched=false
    end

    janeSection:Toggle({ Title="Enable Patch", Desc="Auto-fire Crystal ability", Value=false, Callback=function(v)
        if vtUnloaded then return end
        vtEnabled=v; local actor=vtGetLocalActor()
        if vtEnabled and not vtPatched and actor then vtApplyPatch(actor) end
    end })
    janeSection:Toggle({ Title="Aimbot", Desc="Auto-aim at nearest killer", Value=false, Callback=function(v)
        if vtUnloaded then return end
        vtAimbotOn=v
        if not vtAimbotOn then vtKillerMotion={}; vtSetShiftLock(false); vtSetMouseBehavior(Enum.MouseBehavior.Default) end
        local actor=vtGetLocalActor()
        if vtAimbotOn and not vtPatched and actor then vtApplyPatch(actor) end
    end })
    janeSection:Slider({ Title="Aim Offset", Desc="Y adjustment on aim target", Step=0.1, Value={Min=-5,Max=5,Default=-0.3}, Callback=function(v) vtAimOffset=v end })
    janeSection:Slider({ Title="Prediction", Desc="Seconds to lead killer", Step=0.01, Value={Min=0,Max=1,Default=0.6}, Callback=function(v) vtPrediction=v end })
    janeSection:Slider({ Title="Hold Duration", Desc="Seconds to hold aim after shot", Step=0.1, Value={Min=0.3,Max=2,Default=0.9}, Callback=function(v) vtHoldDur=v end })
    janeSection:Button({ Title="Unload Jane Patch", Callback=function()
        if vtUnloaded then return end; vtUnloaded=true; vtEnabled=false; vtAimbotOn=false
        vtSetShiftLock(false); vtSetMouseBehavior(Enum.MouseBehavior.Default)
        pcall(function() vtRemovePatch(vtGetLocalActor()) end)
        ui:Notify({ Title="V1perThrow", Content="Patch unloaded.", Icon="check", Duration=3 })
    end })

    -- Actor watcher
    task.spawn(function()
        local lastActor=nil
        while not vtUnloaded do
            task.wait(0.5); if vtUnloaded then break end
            local curActor=vtGetLocalActor()
            if curActor~=lastActor then
                if lastActor~=nil then
                    vtPatched=false; vtCrystalCB=nil; vtKillerMotion={}
                    ui:Notify({ Title="V1perThrow", Content="New round — patch re-applied.", Icon="refresh-cw", Duration=3 })
                end
                lastActor=curActor
                if curActor and vtEnabled then vtApplyPatch(curActor) end
            end
        end
    end)

    secSentinels:Button({ Title="Jane Doe", Callback=function()
        ui:Notify({ Title="Jane Doe", Content="Use the Jane Doe section above!", Duration=3, Icon="info" })
    end })
end
